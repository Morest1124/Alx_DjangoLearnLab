...
from .form import BookForm, UserProfileForm
...
class LibraryDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['books_in_library'] = library.book_set.all()
        return context
...
def register(request):
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            messages.success(request, f'Account created for {username} with role {profile.role}!')
            login(request, user) # Log the user in after registration
            return redirect('list_books') # Redirect to a relevant page after registration
    else:
        user_form = UserCreationForm()
        profile_form = UserProfileForm()
    return render(request, 'relationship_app/register.html', {'user_form': user_form, 'profile_form': profile_form})
